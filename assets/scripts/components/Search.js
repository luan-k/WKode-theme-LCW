    ...
    $.getJSON(
      ...
      (results) => {
        ...
        const categories = [
          {
            name: "newBikes",
            length: results.newBikes.length,
            html: "",
          },
          {
            name: "usedBikes",
            length: results.usedBikes.length,
            html: "",
          },
          {
            name: "products",
            length: results.products.length,
            html: "",
          },
        ];
        ...
        // Sort the categories array in descending order based on length
        categories.sort((a, b) => b.length - a.length);

        for (let cat of categories) {
          finalResult += cat.html;
        }

        this.resultsDiv.html(finalResult);
        this.isSpinnerVisible = false;
        this.initializeCarousel();
        newCardColorLogic();
      }
